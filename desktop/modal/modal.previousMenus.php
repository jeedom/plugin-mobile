    ...
    foreach ($eqLogics as $eqLogic) {
      ...
      if (is_object($userType)) {
        ...
        if ($eqLogic->getConfiguration('appVersion') == 2) {
					...
					$phoneMenus = isset($previousMenus[$eqLogic->getLogicalId()]) ? $previousMenus[$eqLogic->getLogicalId()] : null;
					if($phoneMenus != null){
						echo '<td><select class="menuDefault" eqIdMobile="' . $eqLogic->getId() . '">';
						foreach($phoneMenus as  $index => $phoneMenu){
							$index = $index + 1;
							log::add('mobile', 'debug', 'phoneMenu : ' . json_encode($phoneMenu));
							echo '<option value="default">MENU '.$index.'</option>';
						}
						echo '</select></td>';
          
				}else{
					echo '<td><span class="label label-warning">{{Aucun menu enregistré}}</span></td>';
				}
        } else {
