...
if (typeof jeeDialog !== "undefined") {
  ...
  $("#bt_startTuto").on("click", function () {
    ...
    $("#md_modal")
      .load("index.php?v=d&plugin=mobile&modal=wizard")
      .dialog("open");
  });
    ...
    jeedom.version({
      success: function (version) {
        if (version >= "4.4.0") {
          ...
          $("#md_modal")
            .load("index.php?v=d&plugin=mobile&modal=menuCustom")
            .dialog("open");
        } else {
          $("#div_alert").showAlert({
            message:
  ...
  $("#bt_qrCodev2").on("click", function () {
    $("#md_modal")
      .load("index.php?v=d&plugin=mobile&modal=qrcodev2")
      .dialog("open");
  });
}
...
function clickplugin(id_plugin, name_plugin) {
  if (typeof jeeDialog !== "undefined") {
    jeeDialog.dialog({
      id: "configMobile",
      title:
        "{{Configuration Mobile du Plugin " +
        name_plugin +
        " - Application V1}}",
      contentUrl:
        "index.php?v=d&plugin=mobile&modal=plugin.mobile&plugin_id=" +
        id_plugin,
    });
  } else {
    $("#md_modal").dialog({
...
function clickobject(id_object) {
  if (typeof jeeDialog !== "undefined") {
    jeeDialog.dialog({
      ...
      contentUrl:
        "index.php?v=d&plugin=mobile&modal=object.mobile&object_id=" +
        id_object,
    });
  } else {
    $("#md_modal").dialog({
...
function clickscenario(id_scenario, name_scenario) {
  if (typeof jeeDialog !== "undefined") {
    jeeDialog.dialog({
      ...
      title:
        "{{Configuration Mobile du Scénario " +
        name_scenario +
        " - Application V1}}",
      contentUrl:
        "index.php?v=d&plugin=mobile&modal=scenario.mobile&scenario_id=" +
        id_scenario,
    });
  } else {
    $("#md_modal").dialog({
...
document.querySelectorAll(".btIconClass").forEach((el) => {
  el.addEventListener("click", function (e) {
    ...
    jeedomUtils.chooseIcon(
      function (_icon) {
        let spanIcon = document.querySelector(
          "#spanIconTest" + numElement + '[eqid="' + eqLogicId + '"]',
        );
        spanIcon.innerHTML = "";
        spanIcon.insertAdjacentHTML("beforeend", _icon);
        $("#spanIconTest" + numElement)
          .children("i:nth-child(1)")
          .css("font-size", "60px");
        spanIcon.setAttribute("eqId", eqLogicId);
        $("#spanIconTest" + numElement)
          .children("i:nth-child(1)")
          .attr("id", "area" + numElement);
      },
      {
        icon: _icon,
      },
...
function userSelect(idSelect) {
  ...
  selectorsObject.forEach((selector) => {
    ...
    if (
      idSelector.includes(typeObject) &&
      idSelector.includes(numElement) &&
      eqId == eqLogicId
    ) {
  ...
  if (typeObject == "url") {
    document.querySelector(
      "#urlUser" + numElement + '[eqId="' + eqLogicId + '"]',
    ).style.display = "block";
  } else {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.result == "internalError") {
        el.innerHTML =
          "{{Erreur pas d'adresse interne (voir configuration de votre Jeedom !)}}";
      } else if (data.result == "externalError") {
        el.innerHTML =
          "{{Erreur pas d'adresse externe (voir configuration de votre Jeedom !)}}";
...
function addCmdToTable(_cmd) {
  ...
  if (is_numeric(_cmd.id)) {
    tr +=
      '<a class="btn btn-default btn-xs cmdAction" data-action="configure" title="{{Configuration avancée}}"><i class="fas fa-cogs"></i></a> ';
    tr +=
  ...
  tr += "<td>";
  if (
    init(_cmd.logicalId) !== "notif" &&
    init(_cmd.logicalId) !== "notifCritical"
  ) {
    tr +=
