Plugin permettant d'utiliser l'application Handy Jeedom.

L'application mobile Jeedom nécessite l'installation de ce plugin afin
que la box puisse dialoguer avec l'application Handy.

Konfiguration du plugin Handy 
==============================
...
Pour Hinzufügen un téléphone : **Plugins** → **Kommunikation** → **App
Handy** → **Hinzufügen**
...
-   **Name der mobilen Ausrüstung** : Nom du téléphone

-   **Aktivieren** : Activation de l'accès pour ce mobile

-   **Handyr Typ** : Sélection de l'OS du téléphone (iOS, Android)

-   **Benutzer** : Benutzer associé à cet accès
...
Konfiguration des plugins et commandes que reçoit l'app 
=======================================================

Après l'initialisation du Plugin Handy vous avez la possibilité de
...
Konfiguration de l'application Handy 
=====================================
...
Au 1er lancement de l'application Handy, un tutorial vous sera proposé
...
il suffit de flasher le QRcode présent sur le plugin App Handy dans
...
main les adresses IP interne et externe de votre Jeedom. Diese Option
...
Les Génériques Types dans le plugin Handy 
------------------------------------------
...
Bild                           | Typ Gattung               | Partie Dev plugin            | Beschreibung          |
:-----------------------------: | :--------------------------- | :--------------------------- | :------------------: |
![SMOKE](../images/SMOKE.jpg)   | `Fumée`<br/>`Temperatur`(optionnel)|`SMOKE`<br/>`TEMPERATURE`(optionnel) | Permet d'avoir son capteur de fumée complet sur une seule ligne.
...
Bild                                 | Typ Gattung               | Partie Dev plugin            | Beschreibung          |
:-----------------------------------: | :--------------------------- | :--------------------------- | :------------------: |
![PRESENCE](../images/PRESENCE.jpg)   | `Présence`<br/>`Temperatur`(optionnel)<br/>`Luminosité`(optionnel)<br/>`Humidité`(optionnel)<br/>`UV`(optionnel)<br/>`Sabotage`(optionnel)|`PRESENCE`<br/>`TEMPERATURE`(optionnel)<br/>`BRIGHTNESS`(optionnel)<br/>`HUMIDITY`(optionnel)<br/>`UV`(optionnel)<br/>`SABOTAGE`(optionnel) | Voir image.
...
> **Spitze**
>
> En lisant ce message, il vous dit quel Typ Gattung manque pour
...
> Kamera. Le souci ne ce reproduira plus avec la 1.0.2, pour supprimer
> le cache sans surprimer l'app, il suffit d'aller dans la configuration
> de votre App Handy et de cliquer sur "supprimer le cache".
